// //  src/Student/StudentEnroll.jsx

// import { AlertCircle, ArrowLeft, ArrowRight, BookOpen, CheckCircle, Sparkles, User, X } from "lucide-react";
// import React, { useState } from "react";
// import { useNavigate } from "react-router-dom"; // ← YE ADD
...
//   const navigate = useNavigate();  // ← YE LINE ADD
...
//       localStorage.setItem('enrollData', JSON.stringify(formData));  // ← Login pre-fill ke liye
      
//       setCurrentStep(3);
      
//       setTimeout(() => {
//         navigate('/login?role=student', { replace: true });  // ← AUTO LOGIN PAGE
//       }, REDIRECT_DELAY_MS);
...
//         <div className={`max-w-xl w-full ${colors.surface} backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/50 p-6 relative z-10 hover:shadow-blue-200/50 transition-shadow duration-500`}>
//           <button onClick={handleBackToHome} className="absolute top-3 right-3 p-1.5 rounded-full hover:bg-gray-100 transition-all hover:rotate-90 duration-300">
//             <X className="w-4 h-4 text-gray-500" />
//           </button>

//           <div className="text-center mb-6">
//             <div className={`w-16 h-16 mx-auto bg-gradient-to-br ${colors.primary} rounded-xl flex items-center justify-center shadow-xl mb-4 relative overflow-hidden group`}>
//               <span className="text-3xl relative z-10">🎓</span>
//               <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-500"></div>
//             </div>

//             <h1 className={`text-2xl font-bold bg-gradient-to-r ${colors.primary} bg-clip-text text-transparent mb-2 flex items-center justify-center gap-2`}>
//               Ready to Start Your Journey? 
//               <Sparkles className="w-5 h-5 text-yellow-500 animate-pulse" />
//             </h1>
//             <p className={`text-sm ${colors.muted} max-w-md mx-auto`}>
//               Join <span className="font-bold text-blue-600">50,000+</span> students transforming their careers with expert-led courses
//             </p>
//           </div>

//           <div className="grid sm:grid-cols-3 gap-3 mb-6">
//             {[
//               { bg: "from-blue-50 to-blue-100", border: "border-blue-200", gradient: colors.primary, title: "Expert Mentors", desc: "From FAANG", icon: "👨‍🏫" },
//               { bg: "from-yellow-50 to-orange-50", border: "border-yellow-200", gradient: colors.accent, title: "Live Projects", desc: "Build portfolio", icon: "💼" },
//               { bg: "from-emerald-50 to-emerald-100", border: "border-emerald-200", gradient: "from-emerald-500 to-emerald-600", title: "Job Support", desc: "95% placement", icon: "🎯" }
//             ].map((item, i) => (
//               <div key={i} className={`p-3 bg-gradient-to-br ${item.bg} rounded-xl border ${item.border} hover:scale-105 transition-transform duration-300 cursor-pointer group`}>
//                 <div className={`w-8 h-8 bg-gradient-to-br ${item.gradient} rounded-lg flex items-center justify-center mb-2 group-hover:rotate-12 transition-transform duration-300`}>
...
//         <div className={`max-w-2xl w-full ${colors.surface} backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/50 p-6 relative z-10 max-h-[95vh] overflow-y-auto`}>
//           <div className="mb-6">
//             <div className="flex items-center justify-between mb-4">
//               <button onClick={() => setCurrentStep(1)} className="p-1.5 rounded-full hover:bg-gray-100 transition-all hover:-translate-x-1 duration-300">
//                 <ArrowLeft className="w-5 h-5 text-gray-600" />
//               </button>
//               <div className="flex-1 text-center">
//                 <h2 className={`text-xl font-bold bg-gradient-to-r ${colors.primary} bg-clip-text text-transparent flex items-center justify-center gap-2`}>
//                   Enrollment Form 
//                   <span className="text-lg">📋</span>
//                 </h2>
//                 <p className={`${colors.muted} text-xs mt-1`}>Just a few details to get you started</p>
//               </div>
//               <div className="w-8"></div>
//             </div>

//             <div className="w-full bg-gray-200 rounded-full h-1.5 mb-4 overflow-hidden">
//               <div className={`bg-gradient-to-r ${colors.primary} h-1.5 rounded-full transition-all duration-500 relative`} style={{width: '50%'}}>
...
//                 />
//                 {formErrors.fullName && <p className="text-red-600 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{formErrors.fullName}</p>}
//               </div>
...
//                   />
//                   {formErrors.email && <p className="text-red-600 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{formErrors.email}</p>}
//                 </div>
...
//                   />
//                   {formErrors.phone && <p className="text-red-600 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{formErrors.phone}</p>}
//                 </div>
//               </div>
...
//                   />
//                   {formErrors.dob && <p className="text-red-600 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{formErrors.dob}</p>}
//                 </div>
...
//                   />
//                   {formErrors.city && <p className="text-red-600 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{formErrors.city}</p>}
//                 </div>
//               </div>
//             </div>
...
//                 </select>
//                 {formErrors.course && <p className="text-red-600 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{formErrors.course}</p>}
//               </div>
...
//                 </select>
//                 {formErrors.education && <p className="text-red-600 text-xs mt-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" />{formErrors.education}</p>}
//               </div>
...
//                 ) : (
//                   <>
//                     <span className="relative z-10">Complete Enrollment</span>
//                     <ArrowRight className="w-5 h-5 group-hover:translate-x-2 transition-transform relative z-10" />
//                   </>
//                 )}
//               </button>
...
//           <p className={`text-center text-xs ${colors.muted} mt-4 flex items-center justify-center gap-1`}>
//             <span>🔒</span>
...
//         <div className={`max-w-xl w-full ${colors.surface} backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/50 p-6 text-center relative z-10`}>
//           <div className={`w-20 h-20 mx-auto bg-gradient-to-br ${colors.primary} rounded-full flex items-center justify-center shadow-xl mb-4 animate-bounce relative`}>
//             <CheckCircle className="w-10 h-10 text-white" />
//             <div className="absolute inset-0 bg-white/20 rounded-full animate-ping"></div>
//           </div>

//           <h1 className={`text-2xl font-bold bg-gradient-to-r ${colors.primary} bg-clip-text text-transparent mb-2`}>
//             Enrollment Successful! 🎉
//           </h1>
//           <p className={`text-base ${colors.muted} mb-2 max-w-md mx-auto`}>
//             Welcome aboard, <span className="font-bold text-emerald-600">{formData.fullName}</span>!
//           </p>
...
//               <div className="absolute inset-0 w-10 h-10 border-4 border-emerald-200 border-b-emerald-600 rounded-full animate-spin" style={{animationDirection: 'reverse'}}></div>
//             </div>
//           </div>
...
//           </button> */}
//           <button
//   onClick={() => navigate('/login?role=student')}  // ← YE LINE REPLACE
//   className={`bg-gradient-to-r ${colors.primary} text-white py-3.5 px-6 rounded-xl font-semibold text-base shadow-xl hover:shadow-2xl hover:shadow-blue-500/40 transition-all duration-300 flex items-center justify-center gap-2 mx-auto group relative overflow-hidden`}
// >
//   <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
//   <span className="relative z-10">Go to Dashboard Now</span>
...
//             </h3>
//             <div className={`text-xs ${colors.muted} space-y-2 text-left max-w-sm mx-auto`}>
//               {[
//                 { icon: "✉️", text: "Check your email for course access details" },
//                 { icon: "👤", text: "Complete your profile in the dashboard" },
//                 { icon: "💬", text: "Join our community Discord server" }
//               ].map((item, i) => (
//                 <div key={i} className="flex items-start gap-2 p-2 bg-white/50 rounded-lg hover:bg-white transition-colors">
  ...
  if (currentStep === 3) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 flex items-center justify-center p-3 relative overflow-hidden">
        ...
        <div className={`max-w-2xl w-full ${colors.surface} backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/50 p-6 relative z-10`}>
          <div className="mb-6">
            <div className="flex items-center justify-between mb-4">
              <button onClick={() => setCurrentStep(2)} className="p-1.5 rounded-full hover:bg-gray-100 transition-all hover:-translate-x-1 duration-300">
                <ArrowLeft className="w-5 h-5 text-gray-600" />
              </button>
              <div className="flex-1 text-center">
                <h2 className={`text-xl font-bold bg-gradient-to-r ${colors.primary} bg-clip-text text-transparent flex items-center justify-center gap-2`}>
                ...
                <p className={`${colors.muted} text-xs mt-1`}>Secure & encrypted payment gateway</p>
              </div>
              <div className="w-8"></div>
            </div>

            <div className="w-full bg-gray-200 rounded-full h-1.5 mb-4 overflow-hidden">
              <div className={`bg-gradient-to-r ${colors.primary} h-1.5 rounded-full transition-all duration-500 relative`} style={{width: '75%'}}>
                <div className="absolute inset-0 bg-white/30 animate-pulse"></div>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border-2 border-yellow-200 rounded-xl p-4 mb-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-base font-bold text-gray-800">Course Fee</h3>
                <div className="text-right">
                  <p className="text-xs text-gray-500 line-through">₹50,000</p>
                  <p className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent">₹29,999</p>
                </div>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full font-medium">40% OFF 🎉</span>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {paymentMethods.map((method) => (
                ...
                >
                  {method.tag && (
                    <span className={`absolute -top-2 -right-2 text-xs font-bold px-2 py-0.5 rounded-full bg-gradient-to-r ${method.gradient} text-white shadow-lg`}>
                      {method.tag}
                    </span>
                  )}
                  
                  <div className="flex items-start gap-3">
                    <div className={`w-12 h-12 bg-gradient-to-br ${method.gradient} rounded-xl flex items-center justify-center shadow-lg text-2xl`}>
                      {method.icon}
                    </div>
                    <div className="flex-1">
          ...
          <div className="mt-4 space-y-2">
            <p className={`text-center text-xs ${colors.muted} flex items-center justify-center gap-1`}>
              <span>🔒</span>
