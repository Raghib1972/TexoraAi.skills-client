...
export default function ApplyTrainer() {
  ...
  const isPage1Valid = formData.name && 
                       formData.email && 
                       formData.password && 
                       formData.linkedin && 
                       formData.country && 
  ...
  return (
    <div className={darkMode ? 'dark' : ''}>
      <div className={`min-h-screen ${darkMode ? 'bg-gradient-to-br from-gray-900 via-black to-gray-900' : 'bg-gradient-to-br from-gray-50 via-white to-gray-50'} transition-colors duration-300`}>
        {/* Header */}
        <header className={`border-b ${darkMode ? 'border-gray-800 bg-black/50' : 'border-gray-200 bg-white/50'} backdrop-blur`}>
        ...
        <div className="max-w-3xl mx-auto px-6 py-12">
          ...
          {/* Form Container */}
          <div className={`${darkMode ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200'} backdrop-blur border rounded-2xl p-8 shadow-2xl transition-colors duration-300`}>
            {currentPage === 1 && (
              <div>
                <div className="mb-8">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      What country do you live in? <span className="text-red-500">*</span>
                    </label>
                    <select
                      name="country"
                  ...
                  <div>
                    <label className={`block text-sm font-medium mb-3 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                      Where do you currently share your expertise? <span className="text-red-500">*</span>
                    </label>
                    <div className="space-y-2">
                      {['Blog', 'Newsletter', 'YouTube', 'Podcast', 'Social Media', 'Online courses'].map((option) => (
                        <label key={option} className={`flex items-center gap-3 cursor-pointer p-3 rounded-lg ${darkMode ? 'hover:bg-gray-800/50' : 'hover:bg-gray-100'} transition`}>
                          <input
                            type="checkbox"
                ...
                <div className={`flex items-center justify-end mt-8 pt-6 border-t ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
                  <button
                    onClick={handleNext}
            ...
            {currentPage === 2 && (
              <div>
                ...
                <div className={`flex items-center justify-between mt-8 pt-6 border-t ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
                  <button
                    onClick={handleBack}
            ...
            {currentPage === 3 && (
              <div>
                ...
                <div className={`${darkMode ? 'bg-black/50' : 'bg-gray-50'} rounded-lg p-6 space-y-4 mb-6`}>
                  <div>
                    <div className={`text-sm ${colors.muted} mb-1`}>Name</div>
                    <div className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>{formData.name || '—'}</div>
                  </div>
                  <div>
                    <div className={`text-sm ${colors.muted} mb-1`}>Email</div>
                    <div className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>{formData.email || '—'}</div>
                  </div>
                </div>

                <div className={`flex items-center justify-between pt-6 border-t ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
                  <button
                    onClick={handleBack}
                    className={`flex items-center gap-2 px-6 py-3 ${darkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-gray-200 hover:bg-gray-300'} rounded-lg transition ${darkMode ? 'text-white' : 'text-gray-900'}`}
                  >
