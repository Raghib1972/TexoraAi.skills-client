        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
              ...
              <div className="flex items-center gap-1">
                <Clock size={16} />
                <span>{courseData.liveSessions} live sessions • {courseData.totalLessons} lessons</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800 mb-6">
          <p className="text-gray-600 dark:text-gray-400">
            This comprehensive syllabus covers all aspects of {courseData.title.toLowerCase()}. Each week includes live sessions, 
            self-paced lessons, and hands-on projects to ensure you master the material.
          </p>
        </div>

        <div className="space-y-3">
          {syllabus.map((week, index) => (
            <div key={index} className="border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden bg-white dark:bg-zinc-900">
              <button
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <p className="font-bold text-lg text-gray-900 dark:text-white">{week.week}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{week.dates}</p>
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mt-1">{week.title}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
              ...
              {expandedWeek === index && (
                <div className="p-6 space-y-4 bg-white dark:bg-zinc-900">
                  ...
                  {week.sessions && week.sessions.map((session, idx) => (
                    <div key={idx} className="flex items-start gap-4 p-4 bg-gray-50 dark:bg-zinc-800 rounded-xl hover:shadow-md transition">
                      {session.type === 'live' ? (
        ...
        <div className="mt-8 bg-gradient-to-r from-blue-50 to-emerald-50 dark:from-blue-950/30 dark:to-emerald-950/30 rounded-2xl p-6 border border-blue-200/50 dark:border-blue-800/50">
          <h3 className="font-bold text-xl text-gray-900 dark:text-white mb-2">Ready to Start Learning?</h3>
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Join {courseData.students} students who are already mastering {courseData.title.toLowerCase()}
          </p>
          <button 
            onClick={() => navigate(-1)}
